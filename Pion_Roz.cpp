...
void Pion_Roz::Mutare(Dice die){
	...
	if(req_pos<6){
		...
		actual_posx=pos_casa[6+(casa_die_roll-1)].x;
		actual_posy=pos_casa[6+(casa_die_roll-1)].y;
	...
	if(last_die_roll+die_roll>12){
		casa_die_roll=(last_die_roll+die_roll)-12;
		actual_posx=pos_casa[6+(casa_die_roll-1)].x;
		actual_posy=pos_casa[6+(casa_die_roll-1)].y;
		image->Left=actual_posx;
		image->Top=actual_posy;
...
void Pion_Roz::Iesire_din_casa(Dice die){
	 if(die.GetDiceRoll()==6){

		actual_posx=patrat[14].x;
		actual_posy=patrat[14].y;
		image->Left=actual_posx;
		image->Top=actual_posy;
		pe_tabla=true;
		abia_iesit=true;
